...
**Challenge**: Can you edit the code cell above to subset the sitelist_table by a different primary vegetation type? How about by a different variable (table column)?
...
# View the objects to make sure it worked
site_name
veg
year
ROI
...
*Note: Since we created this plot in base R (rather than through another graphing package, such as "ggplot"), the plot isn't saved as an object- it only appears in the built-in viewer for R. However, you can right click on the plot to copy and paste it elsewhere. If you accidently close a plot and want to see it again, just re-run the code cell that created it.*
...
If you'd like, now try this exercise with another PhenoCam site. Perhaps try a different ecosystem type! To do this, edit the cell near the beginning where you assign the site name, vegtation type, year, and ROI (in the "Download & Plot Canopy Greenness" section). Then be sure to run that cell and all cells below it in sequential order.
